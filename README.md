...
- **Tier 4a MERGED TO MAIN** (11 Mei 2026): commit `abe193c` — Validation Engine sub-branch 4a (C1-C5 Pagu Structure constraints) + UI integration full (dashboard "1.5 Validasi Revisi POK" + 12-card grid + inline indicators di Pagu Anggaran + bidirectional Pagu↔Validasi navigation + row-level scroll/highlight). Feature branch dihapus. **304 tests pass** (201 Tier 3 + 103 Tier 4a). RS Batin Tikal 2025 real-data signal: C1 FAIL catch wrong-mechanism risk (pathway DIPA Halaman III via KAPK, BUKAN revisi POK kewenangan KPA). Lihat `SSOT-REFACTOR-LOG.md §0.9.1-§0.9.7` + `docs/TIER-4-DESIGN.md` + `docs/TIER-4A-PHASE-3-UI-DESIGN.md`.
- **Tier 4b MERGED TO MAIN** (11 Mei 2026): commit `d13be80` — Validation Engine sub-branch 4b (C6 Jenis Belanja, C7 Sumber Dana, C8 LHR APIP, C9 Akun Minus) + UI integration full (cards C6-C9 live + LHR APIP checkbox NEW UX + Submit button triple gating + C1 message UX enhancement batched). Feature branch dihapus. **392 tests pass** (201 Tier 3 + 103 Tier 4a + 88 Tier 4b). Lihat `SSOT-REFACTOR-LOG.md §0.10.1-§0.10.5` + `docs/TIER-4B-DESIGN.md` + `docs/TIER-4B-PHASE-3-UI-DESIGN.md`.
- **Tier 4c Foundation Phase COMPLETE** (11 Mei 2026): Design doc `230ba43` + Konteks 1 TD fix `303df65` + Phase 1.5 types narrow `857e98c` + governance docs sync. Owner approve all T1-T8 defaults. Implementation work split ke fresh AI session per `OWNER-POLICY-FOR-AI-SESSIONS.md` Addendum v1.1. Lihat `SSOT-REFACTOR-LOG.md §0.11` + `docs/TIER-4C-DESIGN.md`.
- **Tier 4c Phase 2 + Phase 3 COMPLETE** (12 Mei 2026, di feature branch, pre-merge): Implementation `feature/tier-4c-procedural-references` (9 commits ahead of main). C10 SBM (32 tests, first WARN severity amber dot 10-25%) + C11 RPD (45 tests, cross-table + T9 strategy toggle BARU) + C12 Deadline (17 tests, procedural date check). T9 NEW (12 Mei 2026): C11 user-configurable toggle `permisif`/`ketat` per Owner direction "learning by doing" — banner UI di-render BETWEEN ValidationDashboardHeader dan card grid, localStorage persist. **All 12 validators LIVE — Submit button finally ENABLES**. 486 tests pass (392 + 94). TS 8 maintained. Ready Phase 4: Owner Vercel E2E test → squash merge. Lihat `SSOT-REFACTOR-LOG.md §0.11.1a T9 BARU` + `docs/TIER-4C-PHASE-3-UI-DESIGN.md`.
