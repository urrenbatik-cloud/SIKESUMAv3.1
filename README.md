...
- **Tier 4c MERGED TO MAIN** sebagai commit `9174782` (12 Mei 2026) — Validation Engine sub-branch 4c: 3 validators (C10 SBM first WARN severity, C11 RPD cross-table + T9 strategy toggle BARU, C12 Deadline procedural) + UI integration full (cards C10-C12 live + cross-tab nav refactor pagu↔rpd + C11 toggle banner inline). T9 BARU: C11 user-configurable toggle `permisif`/`ketat` per Owner direction "learning by doing". Feature branch `feature/tier-4c-procedural-references` dihapus post-merge cleanup. **All 12 validators LIVE — Submit Revisi POK button ENABLES** for first time in project history. 486 tests pass (392 baseline + 94 Tier 4c). TS 8 maintained. Owner Vercel preview E2E test ✅ APPROVED. Lihat `SSOT-REFACTOR-LOG.md §0.11.1a T9` + `docs/TIER-4C-PHASE-3-UI-DESIGN.md`.
